...
function createTreeView(menu, currentNode) {
  ...
  menu.forEach((el) => {
    ...
    const list = document.createElement('li'); //li 요소담을 리스트

    if (el.children) {
      const checkbox = document.createElement('input');//checkbox 체크박스
      checkbox.type = 'checkbox';

      const span = document.createElement('span');//span 이름
      span.textContent = el.name;

      const ul = document.createElement('ul')//ul 생성

      list.append(checkbox, span, ul);    
