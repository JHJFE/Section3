...
export default function Mypage({ accessToken ,setIsLogin}) {
  ...
  const logoutHandler = () => {
    ...
    axios.delete('http://localhost:4000/logout', { data:{accessToken} })//{ githubUserData, serverResource } 이렇게 담김
      .then((res) => {
      console.log(res)
      setIsLogin(false)
      })
  ...
  useEffect( () => {
    ...
    // isLoading 상태를 false로 업데이트해야 합니다.
     axios.post('http://localhost:4000/userInfo', { accessToken })//{ githubUserData, serverResource } 이렇게 담김
      .then((res) => {
