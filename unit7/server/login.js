...
module.exports = async (req, res) => {
  ...
  const token = generateToken(userInfo, checkedKeepLogin) // access , refresh 모두 담기는 듯?
  const cookieOptions = {
    domain: 'localhost',
    path: '/',
    httpOnly: true,
  ...
  if (checkedKeepLogin) { // true 일때, access refresh 모두 전송
    cookieOptions.maxAge = 24 * 6 * 60 * 10000;
