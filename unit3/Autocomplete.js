...
export const DropDownContainer = styled.ul`
  ...
  > li {
    ...
    &.on{
      ...
      color: white;
    } // 아름님 여기요!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 리스트안에서 on 클래스 가지고 있으면 색상 변경
  }
`;
...
export const Autocomplete = () => {
   ...
   */
  const [hasText, setHasText] = useState(false); // 값의 유무
  const [inputValue, setInputValue] = useState(''); // 입력된 값
  const [options, setOptions] = useState(deselectedOptions); // 연관 검색어 값
  ...
  let handleKeyUp = (e) => { // 구현 성공!

    switch(e.key){// rustic, antique, vintage 라고 했을 때
      case "ArrowDown":
        if(pos < options.length -1 ) {setPos(pre=> pre = pre+1) } // 아래 키 눌리면 인덱스 1 증가
        break;

      case "ArrowUp":
        if(pos > 0 ) setPos(pre=> pre = pre-1) // 위 키 눌리면 인덱스 1 감소
        break;

      case "Enter": // 엔터 누르면
        handleDropDownClick(options[pos]) // 해당 인덱스의 options 요소 가져오고
        setPos(0)// 0으로 초기화
        break;  
    }
  }
  ...
  return (
    <div className='autocomplete-wrapper' onKeyUp={handleKeyUp}>
      <InputContainer>
        ...
        <input onChange={handleInputChange} value = {inputValue} type="text" placeholder='검색어를 입력하세요'/>
      ...
      {inputValue  ? <DropDown options = {options} handleComboBox = {handleDropDownClick} on = {pos}/> : null}
    </div>
  );
};

export const DropDown = ({ options, handleComboBox, on }) => {
  return (
    <DropDownContainer>
      ...
      {options.length !== 0 ?options.map((el,index)=>{
        ...
        return <li  className={on === index? 'on':null} onClick={()=>handleComboBox(el)}>{el}</li>
